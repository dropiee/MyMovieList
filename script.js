...
function displayData() {
  ...
  if (Object.keys(data).includes("Error")) {
    ...
    document.getElementById("search-by-title-error").innerText =
      "Search did not yield results. Please make sure the title is correct.";
...
searchForm.addEventListener("submit", (event) => {
  ...
  if (titleForm.value.trim() == "") {
    document.getElementById("search-by-title-error").style.display = "block";
    document.getElementById("search-by-title-error").innerText =
      "Title parameter must be filled!";
    return console.error("Search did not yield results");
  }
  ...
  async function fetchData() {
    try {
      const response = await fetch(
        "//www.omdbapi.com/?apikey=3ae78589" +
          titleFormPathSegment +
          yearFormPathSegment
      );
...
function addToList() {
  ...
  let newCell3 = createNewCell(
    3,
    document.createTextNode(
      entryStatusOption.options[entryStatusOption.selectedIndex].text
    )
  );
  let newCell4 = createNewCell(
    4,
    document.createTextNode(document.getElementById("date-completed").value)
  );
  let newCell5 = createNewCell(
    5,
    document.createTextNode(
      yourScoreOption.options[yourScoreOption.selectedIndex].text
    )
  );
